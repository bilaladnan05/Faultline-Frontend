...
export default function PRIssuancePage() {
  ...
  const [desc] = useState(`Automated fix for ${incident?.id ?? "FL-101"}. This PR introduces a 5-second connection timeout and an exponential backoff strategy for database reconnections to prevent cascading failures during network instability.`);

  return (
    <div className="flex flex-col flex-1">
      <TopBar
        breadcrumbs={["Incidents", id || "FL-101", "PR Issuance"]}
        action={
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate(`/incidents/${id || "FL-101"}`)}
              className="border border-gray-200 text-gray-600 hover:bg-gray-50 text-sm font-medium px-4 py-1.5 rounded-lg"
            >
              Discard Draft
            </button>
            <button
              onClick={() => { showToast("PR submitted to GitHub successfully!"); navigate(`/incidents/${id}`); }}
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        <button onClick={() => navigate(`/incidents/${id}`)} className="flex items-center gap-1.5 text-sm text-blue-600 hover:underline font-medium">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 flex items-center gap-1.5">
            ...
            Triggered from incident <span className="font-bold text-gray-800">{incident?.id}</span> · {incident?.title}
          </p>
        </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <textarea defaultValue={desc} rows={3} className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm bg-gray-50 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
